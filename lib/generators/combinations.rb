...
module Generators
  # Consonant × vowel syllables, synthesized purely from Unicode — no source page.
  # A combined glyph is consonant Devanagari + the vowel's combining matra, and
  # the IAST replaces the consonant's trailing 'a' with the vowel's IAST. The
  # inherent 'a' vowel is omitted (bare consonants already carry it and live in
  # the basic deck). aM/aH attach the anusvāra/visarga sign to the inherent 'a'
  # (कं, कः); standalone vowel+mark forms live in the anusvāra generator instead.
  class Combinations < Base
    KEY         = "combinations"
    DESCRIPTION = "Consonant × vowel syllables (matras)"
    OUTPUT_TXT  = "sanskrit_combinations_anki.txt"
    OUTPUT_JSON = "combinations.json"

    # Vowels in barakhadi order. The inherent 'a' is omitted.
    VOWEL_IDS = %w[aa i ii u uu RRi RRI LLi e ai o au aM aH].freeze

    # Vowel ID -> combining matra. anusvāra/visarga are not strictly matras but
    # are included: they sit on top of the inherent 'a' rather than replacing it.
    VOWEL_MATRAS = {
      "aa"  => "ा",
      "i"   => "ि",
      "ii"  => "ी",
    ...
    def card(entry)
      ...
      breakdown = components_devanagari(entry).join(" + ")
    ...
    # Anusvāra/visarga are rendered on U+25CC (◌) so the bare mark is visible —
    # the letters.json forms अं/अः include a spurious 'a' and would mislead.
    def components_devanagari(entry)
