...
# Script 4: Generate Anki flashcards for Sanskrit conjunct consonants
# (saṃyuktākṣara — two or more consonants written as a single ligature).
#
# Reads data/letters.json (produced by scrape_sanskrit.rb) and writes:
#   data/conjuncts.json          - every conjunct with components + frequency
#   sanskrit_conjuncts_anki.txt  - Anki import file for the conjuncts
#
# Like the consonant×vowel combinations, conjuncts are deterministic Unicode and
# need no scraping: a conjunct's Devanagari is its component consonants joined by
# the virama U+094D (e.g. प + ् + र = प्र), and its IAST is each component's roman
# with the inherent trailing 'a' dropped from every component except the last
# (pa + ra -> "p" + "ra" = "pra"). This composition was validated against the
# 360-conjunct ligature table on https://en.wikipedia.org/wiki/Devanagari_conjuncts
# (all 89 below matched exactly).
#
# Usage:
#   bundle exec ruby generate_conjuncts_anki.rb
#
# Card layout:
#   Front: conjunct ligature (large)
#   Back:  IAST + component breakdown + corpus frequency
...
CONJUNCTS_JSON = File.join(DATA_DIR, "conjuncts.json")
OUTPUT_FILE    = File.join(__dir__, "sanskrit_conjuncts_anki.txt")
...
def conjunct_devanagari(component_ids, letters_by_id)
...
def conjunct_roman(component_ids, letters_by_id)
...
def build_conjuncts(letters_by_id)
  CONJUNCTS.map do |component_ids, frequency|
    {
      "id"            => component_ids.join("_"),
      "component_ids" => component_ids,
      "devanagari"    => conjunct_devanagari(component_ids, letters_by_id),
      "roman"         => conjunct_roman(component_ids, letters_by_id),
      "frequency"     => frequency
    }
  end
end
...
# --- Main ---
puts "=== Sanskrit Conjuncts Anki Generator ==="
puts
...
puts "Step 1: Building conjunct consonants..."
conjuncts = build_conjuncts(letters_by_id)
write_json(CONJUNCTS_JSON, conjuncts)
puts

puts "Step 2: Writing Anki import file..."
write_anki_file(OUTPUT_FILE, conjuncts, letters_by_id)
