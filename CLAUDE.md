...
`lib/iast_devanagari.rb` is a committed, tested IAST↔Devanagari transliterator, reusable beyond this deck. **IAST→Devanagari is one-to-many**: a homorganic nasal cluster can be written with the explicit conjunct (अहङ्कारः) or an anusvara (अहंकारः), both valid for the same IAST (`ahaṅkāraḥ`). So `to_devanagari` returns *one canonical* spelling (the explicit conjunct — always a `valid_pair?` for its input), while `to_iast` **canonicalises the reverse** direction (Devanagari→IAST, many-to-one) by realising anusvara-before-a-stop as the homorganic nasal (`ं` + क → `ṅk`; before a sibilant/semivowel/`ह` it stays `ṃ`). A curated `(iast, devanagari)` pair is trusted iff `valid_pair?(iast, dev)` (`to_iast(dev) == iast`), which accepts *every* valid spelling — this is why `data/vedanta.json` can store the anusvara Devanagari (अहंकारः) alongside the homorganic IAST (ahaṅkāraḥ). `lib/generators/vedanta.rb` (`--vedanta`) is a pure transform over `data/vedanta.json` (via `Vedanta.load`, `lib/vedanta.rb`): key `vedanta:<iast>`, no audio, `requires_letters? == false`.

`data/vedanta.json` was QA'd in three layers: structural + pair-validity (every `(iast, devanagari)` pair satisfies `IastDevanagari.valid_pair?`, enforced by the committed `test/vedanta_data_test.rb`; the disposable `bootstrap/qa_vedanta.rb` does the same `to_iast(dev) == iast` check), a full visual cross-reference of all 59 PDF pages, and an offline Monier-Williams dictionary cross-check (`bootstrap/mw_crosscheck.py`, ~90% confirmed as real MW headwords; flagged source typos corrected — see `bootstrap/qa_visual_report.md` and `bootstrap/mw_crosscheck_report.md`). Residual fixes were made directly in `data/vedanta.json`.

### The vowel sandhi deck

A fourth deck (`🕉️ Sanskrit Sandhi`, `Anki::SANDHI_DECK`) of 38 cards teaching the five vowel (svara) sandhis plus the avagraha rule — front: two parts in Devanagari, space-separated (देव इन्द्र); back: the combined form, IAST of the parts and the whole, which sandhi applied, a one-line rule explanation, and the **context** (see below); no audio. **Computed, not scraped**, like the combinations/conjuncts decks: there is no source data file at all — the driving constant is `PAIRS` in `lib/generators/sandhi.rb`, a curated list of `[word1, word2, type, context]` in IAST. The pairs were chosen from worked examples verified across Whitney, Emeneau & van Nooten, Pāṇini's sūtras, learnsanskrit.org and the Arsha Vidya handouts (a deep-research pass); the best drill source, Arsha Vidya *Enjoyable Sanskrit Grammar Vol. 2*, is an unrecoverable glyph-font PDF (same problem as the Vedanta PDF) so its pairs were not extracted.

`lib/sandhi.rb` is the pure, tested engine. `Sandhi.join(word1, word2, type)` splits the junction (final vowel of word 1 + initial vowel of word 2), applies the rule, and returns the combined IAST + rule metadata (`RULES`). The `type` is **validated** against the junction via `candidates(v1, v2)`: a mislabelled pair raises `ArgumentError`, so the curated list is self-checking (a pair tagged `:guna` whose junction is actually i+i fails loudly at build time and in tests). The mappings are `LONG` (dīrgha), `GUNA`, `VRDDHI`, `SEMIVOWEL` (yaṇ), `AYADI`, and avagraha. One transform subtlety: yaṇ and ayādi **keep the second vowel** (the combined form appends the whole `word2`), whereas dīrgha/guṇa/vṛddhi/avagraha consume it.

**Context (the internal/compound/external distinction).** The front shows two pieces, and a vowel junction's outcome can depend on whether the pieces are two free words, the members of a compound (*samāsa*), or a stem+affix inside one word. The four general rules (dīrgha, guṇa, vṛddhi, yaṇ) give the same result in all three, but a **word-final diphthong differs by context**: across a word boundary, e/o + a → avagraha (te'pi) and ai/au reduce or retain — the ay/av of ayādi (ne+ana→nayana) only appears in *derivation* (internal sandhi), never between two free words. So each pair carries a fourth field, a context symbol from `CONTEXTS` (`:external`/`:compound`/`:internal`), rendered on the card as "(sandhi between two separate words / within a compound / within a single word)". The two context-bound rules are kept consistent in the data and asserted in the tests: every `:ayadi` pair is `:internal`, every `:avagraha` pair is `:external`. (For the e/o+a junction `candidates` still returns both `:ayadi` and `:avagraha`, so the engine accepts either; the curated label decides and the context tag makes which one explicit.) Most dīrgha/guṇa pairs are compounds — the standard teaching examples — which is honest as long as they aren't presented as sentence-level word pairs. Unattested ḷ junctions (guṇa a+ḷ, yaṇ ḷ→l) are deliberately omitted.

`lib/generators/sandhi.rb` (`--sandhi`) maps each `PAIRS` entry through the engine, regenerates Devanagari from IAST via `IastDevanagari.to_devanagari` (same as Vedanta), and writes `data/sandhi.json` + the import file. Key `sandhi:<type>:<w1>+<w2>`, `requires_letters? == false`, no audio. Add a card by appending a `[w1, w2, type, context]` tuple to its category list — the engine and the validation do the rest.
